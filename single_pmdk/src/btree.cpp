 ...
 * class page
 */
void page::constructor(uint32_t level) {
  hdr.constructor();
...
void page::constructor(PMEMobjpool *pop, page *left, entry_key_t key, page *right,
                  uint32_t level) {
  hdr.constructor();
  for (int i = 0; i < cardinality; i++) {
    records[i].key = LONG_MAX;
...
bool page::remove_key(PMEMobjpool *pop, entry_key_t key) {
...
bool page::remove(btree *bt, entry_key_t key, bool only_rebalance,
            bool with_lock) {
  if (!only_rebalance) {
...
void page::insert_key(PMEMobjpool *pop, entry_key_t key, char *ptr,
